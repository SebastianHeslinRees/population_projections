#' Project fertility rates forward based on a national trend
#'
#' Applies the national rate of change in fertility to a set of local
#' authority AMSRs to produce a fertility probability trajectory.
#'
#' @param jump_off_rates Dataframe. A set of ASMRs for the first projection year.
#' @param future_fertility Dataframe. A national-level ASMR trajectory.
#' @param first_proj_yr Integer. The first projection year.
#' @param n_proj_year Integer. Number of years to project
#' @param npp_var Character. fertility trend variant. Either \code{2018_principal},
...
project_fertility_rates <- function(jump_off_rates, future_fertility, first_proj_yr, n_proj_yr, npp_var="2018_principal"){

  #Test/validate
  check_validate_proj_mort_rates(jump_off_rates, future_fertility, first_proj_yr, n_proj_yr, npp_var)

  final_projection_year <- first_proj_yr + n_proj_yr -1

  future_fertility_probs <- future_fertility %>%
...
check_validate_proj_mort_rates <- function(jump_off_rates,
  ...
  assert_that(is.data.frame(future_fertility),
              msg="future_fertility expects a data frame as input")
  ...
  validate_population(future_fertility, col_aggregation = c("sex","age","year","variant"))
  validate_join_population(jump_off_rates, future_fertility)
