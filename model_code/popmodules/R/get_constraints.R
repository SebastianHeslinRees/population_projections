...
get_constraints <- function(constraint_list, last_proj_yr){
  ...
  constraint_list$constraint_lookup <- readRDS(constraint_list$lookup_path)
  constraint_filter <- constraint_list$constraint_lookup$gss_code
  ...
  if(constraint_list$components$population){
    constraint_list$population_constraint <- get_component_from_file(paste0(path,"population.rds"),
                                                                     last_proj_yr) %>% 
      .make_constraint(constraint_filter, constraint_list$mapping, constraint_list$constraint_lookup)
  }
...
.make_constraint <- function(x, gss_codes, mapping, constraint_areas){
    nm <- last(names(x))
...
validate_get_constraints <- function(constraint_list, last_proj_yr){
  ...
  assert_that(setequal(names(constraint_list), c("constraint_path", "mapping", "lookup_path", "components")),
              msg = "In get_constraints: names(constraint_list) must be: constraint_path, mapping, components, lookup_path")
  ...
  assert_that(is.string(constraint_list$lookup_path), msg = "In get_constraints: constraint_list$lookup_path parameter must be a string")
  ...
  assert_that(file.exists(constraint_list$lookup_path), msg = "In get_constraints: constraint_list$lookup_path file does not exist")
