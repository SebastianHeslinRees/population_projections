...
# returns the asfr structure with fertility curves smoothed where a fit can be found.
# Where a fit cannot be found the data is left unchanged
# Output: output$data is the updated asfr dataframe
#         output$LA_status is a list of 3 - LAs with successful fits on 1st pass, 2nd pass, and failed
#         output$resid_sum is a sum of the squares of the residuals


fit_fertility_rates <- function(asfr.structure, reproducible=T){
        if (reproducible == T) {
                set.seed(42)
        }
        ...
        curve_function<-function(age,m,a,b1,c1,b2,c2){
                ...
                fertility_rate <- a*m*(b1/c1)*(c1/age)^(3/2)*exp(-b1^2*(c1/age + age/c1 -2)) +
        ...
        for (GSS in LAs){
                ...
                # non-linear least squares fit using modified Levenberg-Marquardt algorithm
                        model_output <- nlsLM2(fertility_rate ~ curve_function(age,m,a,b1,c1,b2,c2),
                                               data=data_to_fit,
        ...
        for (GSS in failed_LAs){
                ...
                try({
                        model_outputs <- nlsLM2(fertility_rate ~ curve_function(age,m,a,b1,c1,b2,c2),
                                                data=data_to_fit,
                                                start=grid_start,
                                                control=list(maxiter=200,warnOnly=TRUE))
        ...
        smoothed_data <- lapply(seq_along(coefs),combine_data) %>% 
                rbindlist() %>%
                mutate(sex="F",ID=paste0(age,gss_code,sex)) %>%
                mutate(fertility_rate=ifelse(fertility_rate<0,0,fertility_rate))
...
nlsLM2 <- function(formula, data=parent.frame(), start, jac=NULL, 
        ...
        if (!"package:minpack.lm" %in% search()) stop("load the minpack.lm package before running")
