...
pop <- data.frame( area=c("a","b","c"), pop_col = 100, stringsAsFactors = FALSE)
pop2 <- expand.grid( age=20:23, area=c("a","b","c"), sex=c("f","m"), pop_col = 100, stringsAsFactors = FALSE)
...
test_that("deaths_null creates the expected output", {
  expect_equal(deaths_null(pop, col_aggregation = "area", pop_col = "popn"), deaths, check.attributes=FALSE)
  expect_equal(deaths_null(pop2, col_aggregation = c("age","area","sex"), pop_col = "popn"), deaths2, check.attributes=FALSE)
})

test_that("deaths_null doesn't care about the order of aggregation columns and warns if there are duplicates", {
  expect_equal(deaths_null(pop2, col_aggregation = c("area","age","sex"), pop_col = "popn"), deaths2, check.attributes=FALSE)

  expect_warning(temp <- deaths_null(pop2, col_aggregation = c("area","age","sex","sex"), pop_col = "popn"))
...
test_that("deaths_null handles additional, unused input columns", {
  ...
  expect_equal(deaths_null(pop_in, col_aggregation = "area", pop_col = "popn"), deaths, check.attributes=FALSE)
})
...
test_that("deaths_null handles factors, tibbles and groups", {
  ...
  expect_equal(deaths_null(pop_in, col_aggregation = "area", pop_col = "popn"), deaths_out, check.attributes=FALSE)
...
test_that("deaths_null warns when factor levels don't match the input", {
  ...
  expect_warning( temp <- deaths_null(pop_in, col_aggregation = "area", pop_col = "popn") )
...
test_that("deaths_null deals with non-standard column names", {
  ...
  expect_equal(deaths_null(pop_in, col_aggregation = c("xage", "xarea", "xsex"), pop_col = "popn"), deaths_out, check.attributes = FALSE)
})
...
test_that("deaths_null can give custom numbers per aggregation level", {
  deaths_out <- dplyr::mutate(deaths, deaths = 100)
  expect_equal(deaths_null(pop, col_aggregation = "area", pop_col = "popn", const = 0), deaths)
  expect_equal(deaths_null(pop, col_aggregation = "area", pop_col = "popn", const = 100), deaths_out)
})

test_that("deaths_null hates negative death rates", {
  expect_error(deaths_null(pop, col_aggregation = "area", pop_col = "popn", const = -1))
})
