...
The 2020-based population projections comprise both *variant trend projections* and *housing-led scenario projections*. Trend projections are based on the continuation of past trends in population change while the housing-led projections incorporate considerations of future housing delivery into the the projection process. Trend projections are most appropriate for strategic planning at the regional level while for most other uses the GLA recommends the housing-led projections.
...
c <- round((filter(data, x_axis_col == 2050, colour_col == "High Population")$y_axis_col)/1000000,3)
d <- round((filter(data, x_axis_col == 2050, colour_col == "Low Population")$y_axis_col)/1000000,3)
...
GLA Demography team produces a range of population projections which are used to help inform planning and policy work within the GLA and across London. Projections are updated annually, incorporating the latest data as it becomes available.
...
Four principal variant projections have been produced using different assumptions about future levels of domestic and international migration, they are:
...
Variant projections have been produced in order to assist users in understanding current uncertainty about future population growth. In addition to the four principal variants presented here, five additional variants were produced during the development of the 2020-based projections. Data for all variants has been published to the <a target="_blank"  href="https://data.london.gov.uk/demography/population-and-household-projections/">London Datastore</a>.
...
The same panel of experts was consulted for the 2020-based projections. Their input was helpful in setting the migration assumptions and confirming the general approach taken in the production of this round of projections. A summary of the consultation responses and the recommendations arising is available on the <a target="_blank" href="https://data.london.gov.uk/demography/population-and-household-projections/">London Datastore</a>..
...
Annualised growth over the 30-year projection period 2021-2050 is `r e` persons per year in the Low Population variant. This is in the context of average growth of 94,100 in London over the last decade.
...
Alongside the four main variants the GLA has published an additional five supplementary variants for use in sensitivity analysis. The full range of projections outputs is available on the <a target="_blank" href="https://data.london.gov.uk/demography/population-and-household-projections/">London Datastore</a>.
...
A paper on the models, methodologies and configuration options used in producing these projections is available to download from the London Datastore: <https://data.london.gov.uk/demography/population-and-household-projections/> 
...
The 2020-based projections provide a lower growth trajectory outlook than the 2019-based projections. In the 2019-based projections the Central Range of population in 2050 was 10.516 to 10.915 million, while in 2020 that growth has been revised to between `r a` and `r b`.
...
**ADD 2020 TO CHART - SEE IF IT WORKS**
...
The Central Range projects growth up to 2040 and then a leveling-off/slight decline in the last decade of the projections.
...
```
<div class="source">Source: GLA 2020-based projections, ONS 2014-based household model</div>
...
The 2020-based housing-led projections comprise three scenario projections each of which uses a different housing delivery trajectory.
...
In this scenario, the number of dwellings is forecast increase by `r g` thousand over the 21-year period to 2041. This is an increase of `r h`% over current stocks.
...
In the Housing Targets scenario, the number of dwellings is forecast increase by `r j` million over the 21-year period to 2041. This is an increase of `r k`% over current stocks.
...
The chart shows the 2020-based trend projections Central Range in grey alongside the three housing-led scenarios population trajectories. Note that the migration assumptions in the housing-led scenarios are consistent with the lower bound of the Central Range. This effectively means that differences between the housing-led lines and the lower bound of the Central Range are a result of differences in housing capacity, as applied in the modelling.
