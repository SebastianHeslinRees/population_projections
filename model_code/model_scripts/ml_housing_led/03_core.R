ml_housing_led <- function(start_popn, fertility, mortality, development, ahs, n_proj_yr) {
  ...
  # set up the modules for the model
  ...
  age_on <- age_on_sya
  calc_births <- births_from_popn_fert
  add_births <- add_births_to_aged_popn
  calc_deaths <- deaths_from_popn_mort
  remove_deaths <- remove_deaths
  calc_housing_popn <- simple_popn_from_dev
  add_base_to_dev_popn <- add_base_to_dev_popn
  scale_to_target_popn <- scale_to_target_popn
