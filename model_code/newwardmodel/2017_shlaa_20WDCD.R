...
ward_join <- cbind(as.data.frame(large_sites_points), over(large_sites_points, ward_polygons)) %>%
        select(lhcss_ref, GSS_CODE = WD20CD)
...
large_input <- filter(large_input, !is.na(gss_code_ward)) #TODO there are 4 persmissions that aren't being attached to a ward 
...
oa_to_ward <- fread("C:/Projects_c/regrosser/data/lookups/Output_Area_to_Lower_Layer_Super_Output_Area_to_Middle_Layer_Super_Output_Area_to_Local_Authority_District_(December_2011)_Lookup_in_England_and_Wales.csv") %>% 
        data.frame() %>% 
