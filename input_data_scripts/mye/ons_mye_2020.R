...
message('ONS 2020 MYE')

#-------------------------------------------------------------------------------

#### READ IN

data_location <- "Q:/Teams/D&PA/Data/population_estimates/ons_mid_year_estimates/current_series/mye_2020/"
output_location <- "input_data/mye/2020/"
dir.create(output_location, recursive = TRUE, showWarnings = FALSE)

all_components <- readRDS(paste0(data_location, "mye_2020_components_UK_(2021_geog).rds"))
population <- readRDS(paste0(data_location, "mye_2020_population_estimates_(2021_geog).rds"))
#-------------------------------------------------------------------------------

#SPLIT OUT COMPONENTS INTO SEPARATE DATAFRAMES

components <- unique(all_components$component)
...
population <- population %>% select(gss_code, year, sex, age, popn)
